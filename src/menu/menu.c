...
static void Menu_DrawBack(boolean flash, s32 scroll, u8 r0, u8 g0, u8 b0, u8 r1, u8 g1, u8 b1)
{
	...
	RECT back_dst = {0, -scroll, SCREEN_WIDTH, SCREEN_WIDTH * 4 / 5};
...
void Menu_Load(MenuPage page)
{
	...
	switch (menu.page = menu.next_page = page)
	{
		case MenuPage_Opening:
			...
			menu.page_state.opening.funny_message = ((*((volatile u32*)0xBF801120)) >> 3) % COUNT_OF(funny_messages); //system clock seeding
			break;
		default:
			break;
	}
...
void Menu_Tick(void)
{
	...
	//Fallthrough
		...
		case MenuPage_Main:
		{
			...
			//Draw background
			Menu_DrawBack(
				menu.next_page == menu.page || menu.next_page == MenuPage_Title, menu.scroll >> (FIXED_SHIFT + 1),
				253 >> 1, 231 >> 1, 113 >> 1,
				253 >> 1, 113 >> 1, 155 >> 1
			);
			break;
